...
<html lang="en">
  ...
  <script type="text/javascript">
    function getDate(){
 	   ...
 	   window.location.href="<%=path%>/todayServiceCustomers/selectAll?date="+date+"&status=1";
    }
    //未服务中列表
    function findNotService(){
  	  window.location.href='<%=path%>/todayServiceCustomers/selectAll';
    }
    //已完成
    function findCompletedService(status){
        window.location.href='<%=path%>/todayServiceCustomers/selectCompletedList?status='+status;
    }
    //计划邀约客户页面
    function selectpendingCustomers(){
        window.location.href='<%=path%>/pendingcustomers/page'
    }
    function  detail(archivalNumber){
      var taskId=$("#taskId").val();
  	  var url="<%=path%>/todayServiceCustomers/detail?archivalNumber="+archivalNumber+"&themeTaskId="+taskId
  	  layer.open({
  	      type:2,
  	      title:"本次服务项目",
  	      closeBtn:1, //显示关闭按钮
    ...
    <div class="col-md-12">
      <div class="panel panel-default">
            ...
            <div class="panel-body">
              <!-- Start right -->
              <div class="col-md-12 col-lg-6" style="padding-left:0px">
                <a href='#' class="btn" style="background: none;" onclick="findNotService()">未服务&nbsp;&nbsp;&nbsp;${notService }</a>
                <a href="#" class="btn"  style="background: none"><font color="blue">服务中    &nbsp;&nbsp;&nbsp;     ${inService }</font></a>
    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table id="inService" class="table table-striped table-bordered">
                <thead>
                    <tr>
                          <th>姓名</th>
                          <th>性别</th>
...
$(document).ready(function () {
    var dataTable = $("#inService").dataTable(options).api();
...
var column = [
	...
	"phoneNumber",
	function(data){
		return data.completeStatus+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='detail("+data.archivalNumber+")' style='cursor:pointer'>详情</a>"
	},
	"sourceName"
	];
var options = page("<%=path%>/todayServiceCustomers/findServiceCustomer?taskId="+$("#taskId").val()+"&status=1", column, function (data) {
    return data.data;
}, 'search');
</script>
<script type="text/javascript">
